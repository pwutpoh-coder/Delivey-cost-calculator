...
with col1:
    ...
    formatted_costs = []
    formatted_costs_per_tank = []
    for c in breakdown_costs:
        if isinstance(c, (int, float)):
            formatted_costs.append(f"{c:,.2f}")
            if num_tanks > 0:
                c_per_tank = c / num_tanks
                formatted_costs_per_tank.append(f"{c_per_tank:,.2f}")
            else:
                formatted_costs_per_tank.append("0.00")
        else:
            formatted_costs.append(str(c))
            formatted_costs_per_tank.append("-")

    df_breakdown = pd.DataFrame({
        "รายการคำนวณ": breakdown_items,
        "จำนวนเงิน (บาท)": formatted_costs,
        "ราคาต่อถัง (บาท/ถัง)": formatted_costs_per_tank
    })
